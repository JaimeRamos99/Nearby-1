...
class AuthStuff {
    sign(user) {//solo se accede cuando la persona está en firstview, y para llegar a firstview o no hay auth en local storage o está alterado... por eso no es necesario revisar
        ...
        let auth = jwt.sign(jsonAuth, theHandler.jwt(), { expiresIn: '7d' })
    ...
    verify() {
        try {
            ...
            const email = jwt.verify(ls, theHandler.jwt()).email
    ...
    async verifyExpress() {
        try {
            let ls = localStorage.getItem('auth')
            if (ls) {
                const email = await jwt.verify(ls, theHandler.jwt()).email
                if (email !== undefined && email !== null) {
                    return (email)
                } else {
                    return (undefined)
