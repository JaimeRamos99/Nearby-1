...
class Login extends React.Component {
    ...
    handlerSignIn = async () => {
        if ( this.state.pass.pass.length > 5 && this.state.user.trim().length > 0) {//verificación si está registrado y todo bien
            console.log("hola")
            let x = await api.login(this.state.user.trim().toLowerCase(), this.state.pass.pass)
            if (x) {
                console.log(x)
                if (x.error) {
                        ...
                        <Col className="secondColumn" xs={12} sm={12} md={8} lg={8} xl={8}>
                            <div className="containerForm">
                                <div style={{ display: 'flex', flexDirection: 'row', paddingBottom: 10 }}>
                                    <input type="text" className="form-control" placeholder="Email address" aria-label="Recipient's username" aria-describedby="basic-addon2" onChange={(e) => { this.onChangeUser(e.target.value) }} />

                                </div>
                                <div style={{ paddingBottom: 10 }}>
                                    <Password value={this.state.value} className="form-control" placeholder="Nearby password" onChange={(e) => this.onChangePass({ pass: e.target.value })} onKeyDown={this._handleKeyDown} />
                                </div>
                                <div className="containerBoton">
                                    {this.state.loading ?
                                        <Button variant="primary" className="botonEstilo" onClick={() => this.handlerSignIn()} >Verifying data...</Button>
                                        :
                                        <Button variant="primary" className="botonEstilo" onClick={() => this.handlerSignIn()} >Sign in</Button>
